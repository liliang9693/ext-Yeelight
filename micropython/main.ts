...
namespace Yeelight{
    ...
    export function devsetname(parameter: any, block: any) {
        ...
        Generator.addImport("from yeelight import *"); 
        Generator.addCode(`${name} = Bulb(discover_bulbs()[${dev}]["ip"])`);
    ...
    //% R.shadow="range"  R.params.min=0    R.params.max=10    R.defl=0
    //% G.shadow="range"  G.params.min=0    G.params.max=10    G.defl=255
    //% B.shadow="range"  B.params.min=0    B.params.max=10    B.defl=0
    //% NAME.shadow="string"   NAME.defl="bulb" 
